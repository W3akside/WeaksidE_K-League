...
with t1:
    ...
    # 경기결과 표에 가운데 정렬 스타일 적용
    st.dataframe(pd.DataFrame(res1).style.apply(stl_res, axis=1), use_container_width=True, hide_index=True, column_config=res_cfg)
...
with t2:
    ...
    # 경기결과 표에 가운데 정렬 스타일 적용
    st.dataframe(pd.DataFrame(res2).style.apply(stl_res, axis=1), use_container_width=True, hide_index=True, column_config=res_cfg)
