...
r2 = "03.14 14:00,수원삼성,3:0,용인,https://youtu.be/R94v8Y6eD6w|03.14 16:30,안양,1:1,부산,https://youtu.be/R94v8Y6eD6w|03.15 14:00,파주,경기전,김해,"
...
with t1:
    ...
    st.dataframe(pd.DataFrame(k1_data, columns=["순위","로고","팀명","경기수","승점","승","무","패"]).style.apply(stl, axis=1), use_container_width=True, hide_index=True, column_config={"로고": st.column_config.ImageColumn(" "), "팀명": st.column_config.Column(width="medium")})

with t2:
    st.subheader("📅 K2 최신 결과")
    ...
    st.subheader("📊 K2 현재 순위 (17개 팀)")
    st.dataframe(pd.DataFrame(k2_data, columns=["순위","로고","팀명","경기수","승점","승","무","패"]).style.apply(stl, axis=1), use_container_width=True, hide_index=True, column_config={"로고": st.column_config.ImageColumn(" "), "팀명": st.column_config.Column(width="medium")})
