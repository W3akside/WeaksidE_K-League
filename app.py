...
with t1:
    ...
    st.subheader("📊 K1 순위표")
    st.dataframe(pd.DataFrame(k1_d, columns=["순위","로고","팀명","경기수","승점","승","무","패"]).style.apply(stl, axis=1), use_container_width=True, hide_index=True, column_config={"로고": st.column_config.ImageColumn(" "), "팀명": st.column_config.Column(width="medium")})
